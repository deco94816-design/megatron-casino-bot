...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data.startswith('deposit_') or data.startswith('withdraw_') or data.startswith('crypto_') or data.startswith('oxapay_') or data.startswith('addbal_') or data in ['balance_deposit', 'balance_withdraw', 'confirm_withdraw', 'cancel_withdraw', 'crypto_deposit', 'oxapay_deposit']:
            from casino_bot.handlers.cashier_handler import handle_cashier_callback
            return await handle_cashier_callback(update, context)

        if data.startswith('play_') or data.startswith('bet_') or data.startswith('mode_') or data.startswith('points_') or data.startswith('replay_') or data.startswith('demo_') or data in ['back_to_demo_menu', 'cancel_demo']:
            from casino_bot.handlers.point_games_handler import handle_point_games_callback
            return await handle_point_games_callback(update, context)
        ...
        if data.startswith('mines_') or data.startswith('mine_click_'):
            from casino_bot.handlers.mines_handler import handle_mines_callback
            return await handle_mines_callback(update, context)
        ...
        if data.startswith('cf_'):
            from casino_bot.handlers.coinflip_handler import handle_coinflip_callback
            return await handle_coinflip_callback(update, context)
